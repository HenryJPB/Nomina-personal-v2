 ...
 * @author hpulgar
 */
public class ModelViewAsigConceptoEditV2 implements IModelViewConceptoHistEdit {
            ...
            public void onEvent(Event evento) throws Exception {
                ...
                if (evento.getName().equals("onYes")) {
                    try {
                        ...
                        Logger.getLogger(ModelViewAsigConceptoEditV2.class.getName()).log(Level.SEVERE, null, ex);
                    }  // try  // try
                }  // if
            }  // onEvent()  
    ...
    public void btnRefrescar(@BindingParam("codNomina") final Integer codNomina, @BindingParam("nroTrabajador") final Integer nroTrabajador, @BindingParam("codConcepto") final Integer codConcepto ) {
        //NomAsigConceptoDatPK nomAsigConceptoDatPK = new NomAsigConceptoDatPK(ssnRifEmpresa, ssnCodNomina, ssnNroTrabajador, ssnCodConcepto);
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    @Command("btnIniValores")
    @NotifyChange({"nomAsigConceptoDat", "footer"})   //  Good !!! 
    public void btnIniValores(@BindingParam("inicializable") final String inicializable ) {
        if ( inicializable!=null && inicializable.equals("S") ) {
            nomAsigConceptoDat.setCantidad(0.00);
            nomAsigConceptoDat.setMonto(0.00);
            nomAsigConceptoDat.setPorcentaje(0.00);
        }
    }  // btnIniValores().  
